...
export const Timer = (props) => {
    ...
    >
      <View style={styles.timerView}>
        <TouchableOpacity onPress={() => setTimerModalVisible(false)}>
          <Text style={styles.XcloseIcon}>X</Text>
        </TouchableOpacity>
  ...
  return timerModalVisible ? (
    ...
    >
      <TouchableOpacity onPress={() => pauseButton()}>
        <Image
          style={[styles.timerIcons, { marginTop: 1 * scale }]}
          source={paused ? playicon : pauseicon}
        />
      </TouchableOpacity>
      ...
      >
        <Text
          style={{ fontSize: 30, fontFamily: "Raleway-Bold", color: "#9f3201" }}
        >
          {minutes < 10 ? `0${minutes}` : minutes}:
          {seconds < 10 ? `0${seconds}` : seconds}
        </Text>
